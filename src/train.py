...
def main():
    ...
    # Use model_dir if available, otherwise model-dir
    model_dir = getattr(args, 'model_dir', None) or getattr(args, 'model-dir', '/opt/ml/model')
    
    # SageMaker passes S3 path but we need local path
    ...
    # Check for NaN values and handle them with robust imputation
    logger.info("Checking for missing values...")
    if X.isnull().any().any():
        logger.warning("Found NaN values in features. Using SimpleImputer...")
        # Use SimpleImputer to handle all NaN values
        imputer = SimpleImputer(strategy='median')
        X_imputed = imputer.fit_transform(X)
        X = pd.DataFrame(X_imputed, columns=X.columns, index=X.index)
        logger.info("NaN values handled successfully using SimpleImputer")
    
    # Double check - ensure no NaN values remain
    if X.isnull().any().any():
        logger.error("Still have NaN values after imputation!")
        # Drop any remaining NaN rows as last resort
        X = X.dropna()
        y = y[X.index]
        logger.warning(f"Dropped {len(y) - len(X)} rows with remaining NaN values")
    
    logger.info(f"Final dataset shape: {X.shape} (no NaN values)")
    ...
    results = train_models(X_train_scaled, X_val_scaled, X_test_scaled, 
                          y_train, y_val, y_test, model_dir)
